...
public class remove extends javax.swing.JFrame {
 ...
 Database db = loading.db;
    ...
    private void initComponents() {
        ...
        status.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Working", "In-Service", "Scrap" }));
    ...
    private void ReturnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ReturnActionPerformed
        ...
        try {
            if ("working".equals(sta)) {
                String insert1 = "Select category,os,ms_office from asset where lower(asset_tag) like ? and availability='assigned' and handover_date IS NULL and provide_date IS NOT NULL;";
                ...
                pst2.setString(4, "assigned");
                pst2.setString(5, update.n.toUpperCase());
                pst2.execute();

                try {
                    PreparedStatement pst3 = con1.prepareStatement("insert into asset(asset_tag,category,brand,model,os,ms_office,status,availability) values(?,?,?,?,?,?,?,?)");
                    pst3.setString(1, update.n.toUpperCase());
                    pst3.setString(2, category);
                    pst3.setString(3, update.n2);
                    pst3.setString(4, update.n3);
                    pst3.setString(5, os);
                    pst3.setString(6, office);
                    pst3.setString(7,"working");
                    pst3.setString(8,"free");
                    pst3.executeUpdate();

                } catch (Exception e) {
                    System.out.println(e);
                }
            } else if ("in-service".equals(sta)) {
                PreparedStatement st1 = con1.prepareStatement("update asset set status=?, comment=? where upper(asset_tag) like ? and handover_date IS NULL and provide_date IS NOT NULL;");
                st1.setString(1, sta);
                st1.setString(2, comment.getText());
                st1.setString(3,update.n.toUpperCase());
                st1.execute();
            } else if ("scrap".equals(sta)) {
                String insert1 = "Select category,os,ms_office from asset where upper(asset_tag) like ? and availability='history' and handover_date IS NULL and provide_date IS NOT NULL;";
//
//                try {
//                    PreparedStatement inpst = con1.prepareStatement(insert1);
//                    inpst.setString(1,update.n.toLowerCase());
//                    ResultSet result = inpst.executeQuery();
//                    while (result.next()) {
//                        category = result.getString("category");
//                        os = result.getString("os");
